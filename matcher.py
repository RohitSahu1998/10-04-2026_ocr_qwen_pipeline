...
def extract_qwen_items(data, path=""):
    ...
    Robustly handles various dictionary structures from the Qwen engine.
    """
    results = []
    ...
    # CASE: List (recursive search)
    elif isinstance(data, list):
        for i, item in enumerate(data):
            new_path = f"{path}[{i}]"
            results.extend(extract_qwen_items(item, new_path))
            
    # CASE: Raw Literal (Old format fallback)
    else:
        val = str(data).strip()
...
def match_single_page(qwen_page_dict, ocr_page_list):
    ...
    for q in qwen_items:
        if q['claimed_boxes']:
            ...
            print(f"DEBUG: Matched {len(sane_boxes)} tokens for {q['field']} (AI Conf: {q.get('ai_confidence',0):.2f}, OCR Vis: {avg_conf:.2f})")
            ...
            final_output.append({
                ...
                "ai_confidence": float(q.get('ai_confidence', 0.0)),
            ...
            final_output.append({
                ...
                "bbox": None, 
                "ocr_confidence": 0.0,
                "ai_confidence": q.get('ai_confidence', 0.0),
...
def export_to_csv(all_matched_results, output_csv_path="matched_data.csv"):
    with open(output_csv_path, mode='w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["Page", "Field", "Qwen_Value", "AI_Confidence", "OCR_Confidence", "OCR_Matched_Text", "OCR_Bounding_Box", "Qwen_Native_Bounding_Box"])
        for res in all_matched_results:
            writer.writerow([
                res.get('page', 'Unknown'),
                res['field'],
                res['qwen_value'],
                f"{res.get('ai_confidence', 0):.4f}",
                f"{res.get('ocr_confidence', 0):.4f}",
